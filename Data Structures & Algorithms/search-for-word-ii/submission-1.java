...
class Solution {
    ...
    public void dfs(int rowIndex, int columnIndex, char[][] board, List<int[]> elementsVisited, TrieNode node, StringBuilder word) {
        if(rowIndex < 0 || columnIndex < 0 || rowIndex == rowLength || columnIndex == columnLength || !node.children.containsKey(board[rowIndex][columnIndex]) || isElementAlreadyVisited(rowIndex, columnIndex, elementsVisited))
            return;
        int[] indexesOfElementsVisited = new int[] {rowIndex, columnIndex};
        elementsVisited.add(indexesOfElementsVisited);
        ...
        dfs(rowIndex, columnIndex - 1, board, elementsVisited, node, word);
        elementsVisited.removeIf(arr -> Arrays.equals(arr, indexesOfElementsVisited));
